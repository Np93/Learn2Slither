class Snake:
    def __init__(self, initial_position, size):
        ...
        :param initial_position: Liste de tuples représentant
        les segments du serpent [(x, y), ...].
        :param size: Taille du plateau (nombre de cases sur
        un côté, pour un plateau carré).
        """
    ...
    def move(self, direction, grow=False):
        ...
        :param grow: Booléen indiquant si le serpent doit
        grandir (True) ou non (False).
        """
        if not self.alive:
            return False
