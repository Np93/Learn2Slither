...
class Display:
    ...
    def draw_info(self, score, elapsed_time, snake_length, current_session=None, total_sessions=None):
        ...
        self.screen.blit(time_text, (self.board_size * self.cell_size + 20, 60))
    ...
    def show_end_screen(self, title, score, elapsed_time, snake_length, reason):
        ...
        title_color = (0, 255, 0) if title == "Victory" else (255, 0, 0)  # Vert pour victoire, rouge pour Game Over
        title_text = title_font.render(title, True, title_color)
        
        # Calcul pour centrer le titre
        title_rect = title_text.get_rect(center=(self.board_size * self.cell_size // 2, 100))
        self.screen.blit(title_text, title_rect)

        # Raison de la fin
        reason_text = self.font.render(f"Reason: {reason}", True, (255, 255, 255))
        self.screen.blit(reason_text, (self.board_size * self.cell_size // 2 - 100, 200))

        # Score final
        score_text = self.font.render(f"Score: {score}", True, (255, 255, 255))
        self.screen.blit(score_text, (self.board_size * self.cell_size // 2 - 100, 250))

        # Temps écoulé
        time_text = self.font.render(f"Time: {elapsed_time:.1f}s", True, (255, 255, 255))
        self.screen.blit(time_text, (self.board_size * self.cell_size // 2 - 100, 300))

        # Longueur finale
        length_text = self.font.render(f"Length: {snake_length}", True, (255, 255, 255))
        self.screen.blit(length_text, (self.board_size * self.cell_size // 2 - 100, 350))
