...
class Game:
    def __init__(self, board_size=10, display=True,
                 speed=1, victory_condition=10, mode="player"):
        ...
        # Génération du plateau et récupération du serpent et
        # de la direction initiale
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if new_direction and self.board.direction:
                    ...
                    is_reverse = tuple(map(lambda x, y: x + y,
    ...
    def run(self, agent=None, train=False, current_session=None,
        ...
        :param agent: Agent Q-learning si en mode
        `train` ou `model`.
        :param train: Indique si le jeu est en mode
        `train` (True) ou `model` (False).
        :param current_session: Numéro de la session actuelle
        (utilisé en mode train).
    ...
    def handle_end_screen(self, title, reason, elapsed_time):
        ...
        if self.display_enabled and self.display:
            ...
            # Afficher les informations si l'affichage est désactivé
            print(f"{title}!")
    ...
    def reset(self):
        ...
        self.board.snake = self.board.snake
